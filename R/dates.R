#' Convert partial dates from UN-UNK-UNKN format to MM/DD/YYYY format
#'
#' @details The separator character between dates components for the input vector
#' `dates` can be any character other than the letters `c("U", "N", "K")`. `NA`
...
reshape_pdates <- function (dates, output_sep = "/") {
  ...
  mths[which(mths != "UNK")] <- match(mths[which(mths != "UNK")], month.abb)
...
#' Convert full or partial dates from MM/DD/YYYY format to YYYY-MM-DD format
#'
#' @details the date component separator in the input vector `dates` is irrelevant
